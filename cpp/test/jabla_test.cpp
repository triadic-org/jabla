...
// (Targets are passed as std::vector<float> and cast to int -- reuses create_tensor's
// marshaling path. The forward op threads `targets` onto its node so the vjp can reach
// them, since they aren't a tensor input.)
//
...
//
// Reference: softmax([1,2,3]) = [.0900, .2447, .6652]; target 2 -> -log(.6652) ~ 0.4076.
TEST_CASE("cross_entropy: -log(softmax[target]), one row") {
  clear_tensors();
  int logits = create_tensor({1.0f, 2.0f, 3.0f});       // 1 row x 3 cols
  auto v = get_tensor(cross_entropy(logits, {2.0f}, 1, 3));
  REQUIRE(v.size() == 1);
...
TEST_CASE("cross_entropy: averages the per-row losses") {
  ...
  auto v = get_tensor(cross_entropy(logits, {2.0f, 0.0f}, 2, 3));
  REQUIRE(v.size() == 1);
...
// (Pushes the true-class logit down and the rest up -- sums to ~0, since softmax and
// onehot each sum to 1.)
TEST_CASE("cross_entropy_backward = (softmax - onehot)/N") {
  clear_tensors();
  int logits = create_tensor({1.0f, 2.0f, 3.0f});
  auto v = get_tensor(cross_entropy_backward(logits, {2.0f}, 1, 3));
  REQUIRE(v.size() == 3);
  CHECK(v[0] == doctest::Approx(0.0900f).epsilon(0.01));
  CHECK(v[1] == doctest::Approx(0.2447f).epsilon(0.01));
...
// dgamma / dbeta oracle. x=[1,2,3], dy=[1,1,1]; norm=[-1.2247,0,1.2247]:
//   dbeta  = column-sum of dy         = [1, 1, 1]
//   dgamma = column-sum of dy * norm  = [-1.2247, 0, 1.2247]
TEST_CASE("layernorm_gamma_grad / beta_grad = column reductions") {
  clear_tensors();
  int x  = create_tensor({1.0f, 2.0f, 3.0f});   // 1 row x 3 cols
  int dy = create_tensor({1.0f, 1.0f, 1.0f});
  auto dg = get_tensor(layernorm_gamma_grad(x, dy, 1, 3));
  auto db = get_tensor(layernorm_beta_grad(dy, 1, 3));
  REQUIRE(dg.size() == 3);
  REQUIRE(db.size() == 3);
  CHECK(dg[0] == doctest::Approx(-1.2247f).epsilon(0.01));
  CHECK(dg[1] == doctest::Approx(0.0f).epsilon(0.01));
  CHECK(dg[2] == doctest::Approx(1.2247f).epsilon(0.01));
  CHECK(db[0] == doctest::Approx(1.0f));
  CHECK(db[1] == doctest::Approx(1.0f));
  CHECK(db[2] == doctest::Approx(1.0f));
}

// dx oracle -- the coupled x-gradient. Must use NON-uniform dy: uniform dy gives dx=0
// (sum(norm)=0 identically, the vacuous case). x=[1,2,3], gamma=[1,1,1], dy=[1,0,0].
// g=gamma*dy=[1,0,0]; mean(g)=1/3; norm=[-1.2247,0,1.2247]; mean(g*norm)=-0.4082;
// rstd=1.2247. dx = rstd*(g - mean(g) - norm*mean(g*norm)) = [0.2041, -0.4082, 0.2041].
// (Sums to ~0 -- the layernorm gradient always does.)
TEST_CASE("layernorm_backward: coupled dx (non-uniform dy)") {
  clear_tensors();
  int x  = create_tensor({1.0f, 2.0f, 3.0f});
  int g  = create_tensor({1.0f, 1.0f, 1.0f});
  int dy = create_tensor({1.0f, 0.0f, 0.0f});
  auto v = get_tensor(layernorm_backward(x, g, dy, 1, 3));
  REQUIRE(v.size() == 3);
  CHECK(v[0] == doctest::Approx(0.2041f).epsilon(0.02));
  CHECK(v[1] == doctest::Approx(-0.4082f).epsilon(0.02));
  CHECK(v[2] == doctest::Approx(0.2041f).epsilon(0.02));
  CHECK(v[0] + v[1] + v[2] == doctest::Approx(0.0f).epsilon(0.01));  // grad sums to 0
}
...
// gets a gradient. (indices passed as std::vector<float>, cast to int at the boundary.)
//
// Reference forward: W = [[1,2],[3,4],[5,6]] (vocab 3, dim 2);
// embedding(W, [2,0]) = [[5,6],[1,2]].
TEST_CASE("embedding: gathers W rows by index") {
  clear_tensors();
  int w = create_tensor({1.0f, 2.0f,
                         3.0f, 4.0f,
                         5.0f, 6.0f});                   // vocab 3 x dim 2
  auto v = get_tensor(embedding(w, {2.0f, 0.0f}, 2));    // rows 2 then 0
  REQUIRE(v.size() == 4);
...
TEST_CASE("embedding_backward: scatter-add accumulates on repeated index") {
  ...
  auto v = get_tensor(embedding_backward(dy, {0.0f, 0.0f, 1.0f}, 3, 2));
  REQUIRE(v.size() == 6);
